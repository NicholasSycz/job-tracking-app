...
const DashboardView: React.FC<Props> = ({ applications, onEdit, currentGoal }) => {
  const now = new Date();
  ...
  const monthlyCount = monthlyApplications.length;
  const goalTarget = currentGoal?.target ?? 25;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm transition-colors">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {recent.length === 0 ? (
              ...
              recent.map((job) => (
                <div key={job.id} onClick={() => onEdit(job)} className="group py-3 sm:py-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors cursor-pointer -mx-6 px-6">
                  <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
                    <div className="w-9 h-9 sm:w-10 sm:h-10 shrink-0 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-bold text-slate-500 dark:text-slate-400 group-hover:bg-white dark:group-hover:bg-slate-700 transition-colors text-sm sm:text-base">
                      {job.company.charAt(0)}
                    </div>
                    <div className="min-w-0">
                      <h4 className="font-semibold text-slate-800 dark:text-slate-200 leading-tight group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors text-sm sm:text-base truncate">{job.role}</h4>
                      <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 truncate">{job.company}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 sm:gap-6 shrink-0 ml-2">
                    <div className="text-right">
                      <StatusBadge status={job.status} />
                      <p className="text-xs text-slate-400 dark:text-slate-500 mt-1 hidden sm:block">{new Date(job.dateApplied).toLocaleDateString()}</p>
                    </div>
                    <ChevronRight className="text-slate-300 dark:text-slate-700 group-hover:text-slate-500 dark:group-hover:text-slate-400 transition-colors hidden sm:block" size={18} />
                  </div>
                </div>
              ))
            )}
        ...
        {/* Motivational Card */}
        <div className="max-h-max bg-gradient-to-br from-emerald-600 to-teal-700 rounded-2xl p-5 sm:p-8 text-white flex flex-col justify-between shadow-lg shadow-emerald-200 dark:shadow-none">
          <div>
          ...
          <div className="space-y-4">
             <div className="bg-white/10 p-4 rounded-xl backdrop-blur-sm border border-white/10">
                <p className="text-xs uppercase tracking-wider font-bold opacity-70 mb-1">{monthName} Goal</p>
                <div className="flex justify-between items-end">
                  <span className="text-2xl font-bold">{monthlyCount} / {goalTarget}</span>
                  <span className="text-xs opacity-70 font-semibold">{Math.min(100, (monthlyCount / goalTarget) * 100).toFixed(0)}%</span>
                </div>
                <div className="w-full bg-white/20 h-2 rounded-full mt-2 overflow-hidden">
                  <div className="bg-white h-full rounded-full transition-all duration-1000" style={{ width: `${Math.min(100, (monthlyCount / goalTarget) * 100)}%` }}></div>
                </div>
             </div>
          </div>
        </div>
...
const StatCard = ({ title, value, icon, color }: { title: string, value: number, icon: React.ReactNode, color: string }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-950 p-4 sm:p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
      <div className={`w-10 h-10 sm:w-12 sm:h-12 ${lightColors[color]} ${darkColors[color]} rounded-xl flex items-center justify-center mb-3 sm:mb-4 transition-colors`}>
        {icon}
      </div>
      <p className="text-xs sm:text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">{title}</p>
