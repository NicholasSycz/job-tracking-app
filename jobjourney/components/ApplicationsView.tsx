...
type SortOption = 'date-desc' | 'date-asc' | 'company-asc' | 'company-desc' | 'status';
...
interface Props {
  ...
  onBulkStatusUpdate?: (ids: string[], status: ApplicationStatus) => Promise<void>;
...
const ApplicationsView: React.FC<Props> = ({ applications, onEdit, onDelete, onBulkDelete, onBulkStatusUpdate, deletingIds = new Set(), isLoading = false }) => {
  ...
  const sortedAndFiltered = useMemo(() => {
    let result = applications.filter(app => filter === 'All' ? true : app.status === filter);

    switch (sortBy) {
      case 'date-desc':
        result.sort((a, b) => new Date(b.dateApplied).getTime() - new Date(a.dateApplied).getTime());
        break;
      case 'date-asc':
        result.sort((a, b) => new Date(a.dateApplied).getTime() - new Date(b.dateApplied).getTime());
        break;
      case 'company-asc':
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 text-sm font-medium hover:border-emerald-300 dark:hover:border-emerald-700 transition-colors"
            >
              <ArrowUpDown size={14} />
            ...
            {showSortMenu && (
              <>
                ...
                <div className="absolute right-0 mt-2 w-44 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl shadow-lg z-20 py-1 animate-in fade-in zoom-in-95 duration-150">
          ...
          <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 text-sm">
            <Filter size={16} />
            <span>Showing {sortedAndFiltered.length} applications</span>
          </div>
        </div>
      </div>
